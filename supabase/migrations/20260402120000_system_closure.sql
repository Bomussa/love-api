...
BEGIN
  ...
  v_expected_clinic := (v_q.path ->> (v_q.current_step - 1))::uuid;
  IF p_doctor_clinic_id <> v_expected_clinic THEN RAISE EXCEPTION 'forbidden_clinic'; END IF;
  ...
  IF v_next_status = 'DONE' THEN
    UPDATE clinics SET current_load = GREATEST(0, current_load - 1) WHERE id = v_q.clinic_id;
  END IF;
