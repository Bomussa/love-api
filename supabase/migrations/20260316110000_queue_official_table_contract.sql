-- Queue contract hardening migration
-- Source of truth: public.queues

BEGIN;

-- 1) Ensure canonical columns used by queue enter/call/status paths
...
SET
  ...
  queue_date = COALESCE(queue_date, (entered_at AT TIME ZONE 'UTC')::date, CURRENT_DATE),
  status = CASE
...
-- 2) Migrate data from legacy table public.queue if it exists
DO $$
BEGIN
  IF EXISTS (
...
-- 3) Migrate data from legacy table public.unified_queue if it exists
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
...
-- 4) Query indexes for status / call / enter workloads
CREATE INDEX IF NOT EXISTS idx_queues_enter_lookup
  ON public.queues (clinic_id, patient_id, queue_date)
  WHERE status IN ('waiting', 'called', 'serving');

CREATE INDEX IF NOT EXISTS idx_queues_call_lookup
  ON public.queues (clinic_id, queue_date, status, queue_number_int, display_number)
  WHERE status IN ('waiting', 'called', 'serving');

CREATE INDEX IF NOT EXISTS idx_queues_status_lookup
  ON public.queues (clinic_id, queue_date, status, entered_at DESC);
