...
serve(async (req: Request) => {
  ...
  try {
    ...
    // تسجيل الخطأ في Audit Log (with timeout protection)
    try {
      const serviceClient = createServiceClient();
      const logPromise = serviceClient.from('audit_log').insert({
        action: 'QUEUE_ENGINE_ERROR',
        payload: { error: err.message || String(err), timestamp: new Date().toISOString() },
      });
      
      // Add timeout to prevent hanging on DB failure
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Logging timeout')), 2000)
      );
      
      await Promise.race([logPromise, timeoutPromise]).catch(logErr => {
        console.error('Failed to log error (non-critical):', logErr);
      });
    } catch (logErr) {
