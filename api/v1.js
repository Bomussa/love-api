...
function checkRateLimit(bucket, key, limit, windowMs) {
  ...
  const active = records.filter((ts) => base - ts <= windowMs);
...
export async function validateTransition(queue, nextStatus, nextStep) {
  ...
  const allowedNext = allowed[queue.status] || [];
  if (!allowedNext.includes(nextStatus)) throw new Error(`invalid transition ${queue.status} -> ${nextStatus}`);
}
...
async function bootRecovery(sb) {
  ...
  await sb.from('queues')
    .update({ status: 'WAITING', updated_at: now })
    .eq('status', 'IN_PROGRESS')
    .lt('updated_at', cutoff);

  await sb.from('queues')
    .update({ called_at: null, updated_at: now })
    .eq('status', 'WAITING')
    .lt('called_at', cutoff)
    .not('called_at', 'is', null);

  await sb.rpc('fn_recalculate_clinic_load');
  BOOT.recovered = true;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (pathname === '/api/v1/queue/create' && method === 'POST') {
      ...
      if (idempotencyKey) {
        const { error: insertError } = await sb.from('idempotency_keys').insert({
          key: idempotencyKey,
          response: payload,
          created_at: nowIso(),
        });
        ...
        if (insertError) {
          const { data: stored } = await sb.from('idempotency_keys')
            .select('response')
    ...
    if (pathname === '/api/v1/queue/advance' && method === 'POST') {
      ...
      const { queueId, doctorClinicId, version } = body;
      if (!queueId || !doctorClinicId || version === undefined) return reply(400, { success: false, error: 'queueId, doctorClinicId, version required' });
      if (!checkRateLimit(RATE_WINDOWS.advanceByDoctor, String(doctorClinicId), 60, 60_000)) {
...
export function _testOnly_rateLimit(bucketName, key, limit, windowMs) {
  return checkRateLimit(RATE_WINDOWS[bucketName], key, limit, windowMs);
}
