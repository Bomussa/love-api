...
test('migration is standalone and contains required base schema', () => {
  ...
  assert.match(migration, /CREATE TABLE IF NOT EXISTS queue_logs/i);
  assert.match(migration, /ADD COLUMN IF NOT EXISTS patient_id TEXT/i);
});
...
test('migration enforces constraints and duplicate-safe atomic creation with retry', () => {
  ...
  assert.match(migration, /status_check[\s\S]*WAITING','CALLED','IN_PROGRESS','DONE','CANCELLED/i);
...
test('api idempotency returns stored response directly and stores full payload', () => {
  ...
  assert.match(api, /\/api\/v1\/queue\/call/);
  assert.match(api, /\/api\/v1\/doctor\/queue/);
  assert.match(api, /\/api\/v1\/admin\/doctor-dashboard/);
