...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Canonical handler for POST /api/v1/queue/call now lives in api/v1.js.
    ...
    if ((pathname === '/api/v1/queue/done' || pathname === '/api/v1/queue/complete') && method === 'POST') {
      const clinicId = String(body.clinicId || body.clinic_id || '').trim();
      const patientId = String(body.patientId || body.visitId || '').trim();
      const pin = String(body.pin || body.ticket || '').trim();

      if (!clinicId || !patientId || !pin) {
        return res.status(400).json(formatError('Missing required fields: clinicId|clinic_id, patientId|visitId, pin|ticket', 'MISSING_FIELDS'));
      }
