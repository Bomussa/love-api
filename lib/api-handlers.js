...
function verifyAdminBearerToken(authorizationHeader) {
  const adminSecret = process.env.ADMIN_AUTH_SECRET || process.env.JWT_SECRET;
  if (!adminSecret || typeof authorizationHeader !== 'string' || !authorizationHeader.startsWith('Bearer ')) {
    return false;
  }
  ...
  try {
    ...
    const expMillis = decoded.exp < 1_000_000_000_000 ? decoded.exp * 1000 : decoded.exp;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (pathname === '/api/v1/admin/export-secrets' && method === 'POST') {
      ...
      const hasAdminAccess = verifyAdminBearerToken(req.headers.authorization);
      if (!hasAdminAccess) {
