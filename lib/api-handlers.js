...
<<<<<<< HEAD
import { createAdminToken, verifyAdminBearerToken as verifyAdminBearer, hasValidAdminSecret, verifyPasswordHash } from './admin-auth.js';
=======
import { verifyAdminBearerToken as verifyAdminBearer, hasValidAdminSecret, resolveAdminLoginStatus } from './admin-auth.js';
>>>>>>> master
import {
  parseBody,
  setCorsHeaders,
...
<<<<<<< HEAD
      // Legacy emergency backdoor is disabled by default and must be explicitly enabled.
      const allowLegacyBackdoor = process.env.ALLOW_LEGACY_ADMIN_BACKDOOR === 'true';
      const isLegacyBackdoorMatch = allowLegacyBackdoor && username === 'bomussa' && password === '14490';

      let isValid = isLegacyBackdoorMatch;
      if (!isValid && admin && admin.password_hash) {
        isValid = verifyPasswordHash(password, admin.password_hash);
      }
...
=======
      const loginStatus = resolveAdminLoginStatus({
        ...
        password,
        admin,
        allowLegacyBackdoor: process.env.ENABLE_LEGACY_ADMIN_BACKDOOR === 'true',
      });
...
>>>>>>> master
      ...
      // Generate signed admin token compatible with protected /api/v1/admin/* routes.
