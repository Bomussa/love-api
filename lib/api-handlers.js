...
export default async function handler(req, res) {
  ...
  try {
    ...
    if ((pathname === '/api/v1/pin/verify' || pathname === '/api/v1/pin/validate') && method === 'POST') {
    ...
    // Admin login is handled in api/v1.js to keep a single source of truth.
    ...
    if (pathname === '/api/v1/admin/export-secrets' && method === 'POST') {
      if (process.env.ALLOW_SECRETS_EXPORT !== 'true') {
        return res.status(403).json(formatError('Endpoint disabled', 'ENDPOINT_DISABLED'));
      }

      if (!hasValidAdminSecret(process.env.ADMIN_AUTH_SECRET)) {
        return res.status(503).json(formatError('Server is missing secure ADMIN_AUTH_SECRET configuration', 'SERVER_MISCONFIGURED'));
      }

      const hasAdminAccess = verifyAdminBearerToken(getAuthorizationHeader(req.headers));
      if (!hasAdminAccess) {
        return res.status(401).json(formatError('Unauthorized admin access', 'UNAUTHORIZED_ADMIN'));
      }
      ...
      if (!expectedToken || expectedToken.length < 24) {
        return res.status(503).json(formatError('Export token is not configured securely', 'EXPORT_TOKEN_MISSING'));
      }

      if (!safeConstantTimeEquals(exportToken, expectedToken)) {
