...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (pathname === '/api/v1/admin/login' && method === 'POST') {
      ...
      if (!username || !password) return res.status(400).json(formatError('Missing credentials', 'MISSING_CREDENTIALS'));
      ...
      try {
        ...
        if (error || !admin) return res.status(401).json(formatError('Invalid credentials', 'INVALID_CREDENTIALS'));

        // Basic password check (in real app use bcrypt/argon2)
        ...
        const token = createAdminToken({ id: admin.id, username: admin.username, role: admin.role }, process.env.ADMIN_AUTH_SECRET);
