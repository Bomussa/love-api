...
import { QUEUE_CANONICAL_OPERATIONS, queueCanonicalPath } from './queue-canonical-contract.js';
...
class ApiService {
  ...
  async enterQueue(clinic, user, isAutoEntry = false, retryCount = 0) {
    ...
    try {
      const data = await this.request(queueCanonicalPath(API_VERSION, QUEUE_CANONICAL_OPERATIONS.enter), {
        method: 'POST',
  ...
  async getQueueStatus(clinic) {
    return this.request(`${queueCanonicalPath(API_VERSION, QUEUE_CANONICAL_OPERATIONS.status)}?clinic=${clinic}`);
  }
  ...
  async callNextPatient(clinic) {
    return this.request(queueCanonicalPath(API_VERSION, QUEUE_CANONICAL_OPERATIONS.call), {
      method: 'POST',
  ...
  async resetSystem(adminCode) {
    ...
    try {
      // 1. Clear queues
      await this.supabase.from('queues').delete().neq('id', '00000000-0000-0000-0000-000000000000');
      
      // 2. Reset clinic status
      await this.supabase.from('clinics').update({ 
        pin_code: null, 
        pin_expires_at: null 
      }).neq('id', '00000000-0000-0000-0000-000000000000');
      
      // 3. Clear active pins
      await this.supabase.from('pins').update({ is_active: false }).eq('is_active', true);
