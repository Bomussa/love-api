...
describe('Supabase Client', () => {
  ...
  describe('addToQueue', () => {
    ...
    it('should raise retryable error when conflict persists', async () => {
      ...
      expect(mockSupabaseClient.rpc.mock.calls.length).toBeGreaterThanOrEqual(1);
    });

    it('should support concurrent calls with unique display numbers from RPC responses', async () => {
      const responses = [
        { id: 'q1', display_number: 11 },
        { id: 'q2', display_number: 12 },
        { id: 'q3', display_number: 13 },
      ];
      let i = 0;
      mockSupabaseClient.rpc.mockImplementation(async () => ({ data: responses[i++], error: null }));
