...
describe('Supabase Client', () => {
  ...
  describe('addToQueue', () => {
    ...
    it('should add patient to queue via RPC', async () => {
      ...
      mockSupabaseClient.rpc = vi.fn().mockResolvedValue({ data: mockQueueEntry, error: null });

      const result = await addToQueue(mockSupabaseClient, validPatientData);
      expect(result).toEqual(mockQueueEntry);
      expect(mockSupabaseClient.rpc).toHaveBeenCalledWith('add_to_queue_atomic', expect.objectContaining({
        p_patient_id: '12345',
    ...
    it('should return retryable conflict on uniqueness violation', async () => {
      ...
      await expect(addToQueue(mockSupabaseClient, validPatientData)).rejects.toMatchObject({
        message: 'Queue conflict detected. Please retry the request.',
