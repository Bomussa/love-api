...
export function verifyPasswordHash(password, passwordHash) {
  if (typeof password !== 'string' || !passwordHash || typeof passwordHash !== 'string') {
    return false;
  }

  // Support legacy SHA256 hashes (64 hex characters without salt)
  ...
  // Support salt:hash format (scrypt)
...
/**
 * Maintains compatibility with API handlers that validate an admin password
 * directly from a stored hash.
 */
