...
import { loadSupabaseRuntimeEnv, validateStartupEnv } from './env.js';

globalThis.__VITE_ENV__ = import.meta.env;

validateStartupEnv([
  { key: 'SUPABASE_URL', aliases: ['VITE_SUPABASE_URL'] },
  { key: 'SUPABASE_ANON_KEY', aliases: ['VITE_SUPABASE_ANON_KEY'], description: 'Supabase public key' },
]);

const { supabaseUrl: SUPABASE_URL, supabaseAnonKey: SUPABASE_ANON_KEY } = loadSupabaseRuntimeEnv();

import { QUEUE_CANONICAL_OPERATIONS } from './queue-canonical-contract.js';
...
class SupabaseApiClient {
  ...
  async enterQueue(clinicId, token, priority = 'normal') {
    return this.request(QUEUE_CANONICAL_OPERATIONS.enter.functionName, {
      method: 'POST',
      headers: {
  ...
  async getQueueStatus(clinicId) {
    return this.request(QUEUE_CANONICAL_OPERATIONS.status.functionName, {
      method: 'GET',
      query: `?clinic=${clinicId}`,
    });
  }
  ...
  /**
     * Verify PIN for queue action
     * Canonical function: call-next-patient (pin validated server-side)
     */
  async validatePin(clinicId, pin) {
    return this.request(QUEUE_CANONICAL_OPERATIONS.pinVerify.functionName, {
      method: 'POST',
