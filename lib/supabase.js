...
export async function addToQueue(supabase, patientData) {
  ...
  if (rpcError) {
    console.error('RPC error - cannot safely proceed without atomic number generation:', rpcError);
    throw new Error(`Failed to generate queue number: ${rpcError.message}. Please retry.`);
  }
