...
export function getSupabaseClient(env = process.env) {
  const supabaseUrl = getEnv('SUPABASE_URL', { required: true, aliases: ['NEXT_PUBLIC_SUPABASE_URL'], context: 'Supabase URL resolution' });
  const supabaseKey = getEnv('SUPABASE_SERVICE_ROLE_KEY', { aliases: ['SUPABASE_KEY', 'SUPABASE_ANON_KEY'], context: 'Supabase key resolution' });

  if (!supabaseKey) {
    throw new Error('[ENV] Missing required key: SUPABASE_SERVICE_ROLE_KEY or SUPABASE_KEY or SUPABASE_ANON_KEY');
  }
...
export async function addToQueue(supabase, patientData) {
  // UI fallback contract: on uniqueness conflicts we raise a retryable error code
  // so callers can show a non-fatal message and retry once without breaking UX.
  return withRetry(async () => {
    const {
    ...
    let { data, error } = await supabase.rpc('add_to_queue_atomic', rpcPayload);

    // One immediate retry handles the rare race where uniqueness is enforced
    // before the transaction-visible max(display_number) stabilizes.
    if (error && (error.code === '23505' || error.message?.includes('QUEUE_CONFLICT_RETRYABLE'))) {
      ({ data, error } = await supabase.rpc('add_to_queue_atomic', rpcPayload));
    }

    if (error) {
      console.error('[SUPABASE] addToQueue error:', error);

      if (error.code === '23505' || error.message?.includes('QUEUE_CONFLICT_RETRYABLE')) {
        const conflictError = new Error('Queue conflict detected. Please retry this request.');
