...
export async function addToQueue(supabase, patientData) {
  return withRetry(async () => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      const { data, error } = await supabase.rpc('add_to_queue_atomic', payload);
      ...
      lastError = error;
      // Unique violation => conflict under burst traffic; retry with small jitter.
      if (error.code === '23505') {
        if (attempt < 2) {
        ...
        const conflictError = new Error(`Failed to add to queue: ${error.message}`);
    ...
    console.error('[SUPABASE] addToQueue conflict retries exhausted:', lastError);
    const conflictError = new Error(`Failed to add to queue: ${lastError?.message || 'unknown error'}`);
    conflictError.status = 409;
    throw conflictError;
  });
}
