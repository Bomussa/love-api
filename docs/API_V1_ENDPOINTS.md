...
- تسجيل دخول المشرف في `lib/api-handlers.js` يعتمد فقط على التحقق من `password_hash` لحساب موجود فعليًا في جدول `admins`، ولا يوجد دعم لأي backdoor ثابت داخل الكود.
